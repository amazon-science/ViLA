...
@registry.register_model("vila_lora")
class ViLALoRA(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.fs = FrameSelector(inplanes=257, num_segments=model_frame, frame_num=teacher_frame_num)
        ...
        self.dist_fc_bn = nn.Sequential(
            nn.Linear(32 * model_frame, 32 * teacher_frame_num),
            nn.LayerNorm(32 * teacher_frame_num)
        )
    ...
    def forward(self, samples, temp):
        ...
        # student
        mask = self.fs(image_embeds, temp)
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            #print('inputs_t5_t:', inputs_t5_t.shape, 'inputs_t5', inputs_t5.shape)
            inputs_t5_dist = self.dist_fc_bn(inputs_t5.permute(0,2,1))
            inputs_t5_dist = inputs_t5_dist.permute(0,2,1)
            loss_dist= self.dist_loss(inputs_t5_t, inputs_t5_dist)
            loss = loss + 0.01*loss_dist
    ...
    def generate(
        ...
        mask_output = mask.view(mask.shape[0], mask.shape[1])
        select_frames_idx = torch.topk(mask_output, self.model_frame, dim=-1).indices.tolist()
        sorted_frames_idx = []
        for fms in select_frames_idx:
            sorted_frames_idx.append(sorted(fms))
    ...
    def from_config(cls, cfg):
        ...
        teacher_frame_num = cfg.get("teacher_frame_num", 32)

        print("frame_num:", frame_num)
        print("teacher_frame_num:", teacher_frame_num)
        ...
        model = cls(
            ...
            model_frame=frame_num,
            teacher_frame_num=teacher_frame_num,
        )
