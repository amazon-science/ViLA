...
from lavis.models.blip2_models import InternVideo
...
@registry.register_model("blip2_qa_t5_iv")
class Blip2QAT5IV(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        # wang
        self.visual_encoder = InternVideo.load_model("/scratch_xijun/code/Video/SeViLA/internvideo_ckp/InternVideo-MM-L-14.ckpt", t_size=4)
        self.ln_vision_new = LayerNorm(1408)
        self.visual_proj = nn.Linear(self.visual_encoder.vision_width, 1408)
        if freeze_vit:
        ...
        #!!!!!!#
        # check self.visual_encoder.num_features
        # change all the self.visual_encoder refer
        # !!!!!!#

        # self.visual_encoder, self.ln_vision_new = self.init_vision_encoder(
    ...
    def forward(self, samples):
        ...
        # image_embeds = self.ln_vision_new(self.visual_encoder(image)) # bt, n, c
        # _, n, _ = image_embeds.shape

        # wang internvideo
        image = image.permute(0, 2, 1, 4, 3)  # video: [B, C, T, H, W]
        _, video_features = self.visual_encoder.encode_video(image, return_all_feats=True) # input shape video: [B, C, T, H, W] --> output # [B, C], [L, B, T, C]
        image_embeds = video_features.permute(1,2,0,3) # B, T, L, C
        image_embeds = image_embeds.reshape(-1, image_embeds.shape[2], image_embeds.shape[3]) # BT, L, C
        image_embeds = self.visual_proj(image_embeds)
        image_embeds = self.ln_vision_new(image_embeds)
    ...
    def generate(
        ...
        with torch.cuda.amp.autocast(enabled=(self.device != torch.device("cpu"))):
            #image_embeds = self.ln_vision_new(self.visual_encoder(image))
            # wang internvideo
            image = image.permute(0, 2, 1, 4, 3)  # video: [B, C, T, H, W]
            # input shape video: [B, C, T, H, W] --> output # [B, C], [L, B, T, C]
            _, video_features = self.visual_encoder.encode_video(image,return_all_feats=True)
            image_embeds = video_features.permute(1, 2, 0, 3)  # B, T, L, C
            image_embeds = image_embeds.reshape(-1, image_embeds.shape[2], image_embeds.shape[3])  # BT, L, C
            image_embeds = self.visual_proj(image_embeds)
            image_embeds = self.ln_vision_new(image_embeds)
