...
@registry.register_model("blip2_qa_t5_tt")
class Blip2QAT5TT(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def forward(self, samples):
        ...
        for i in range(len(samples['qa_input'])):
            samples['qa_input'][i] = samples['qa_input'][i] + ' Caption:' + ', '.join(text[self.frame_num*i:self.frame_num*(i+1)]) +'.'
        # tt
    ...
    def generate(
        ...
        text = self.predict_answers(samples)
        for i in range(len(samples['qa_input'])):
            samples['qa_input'][i] = samples['qa_input'][i] + ' Caption:' + ', '.join(
                text[self.frame_num * i:self.frame_num * (i + 1)]) + '.'
        # tt
