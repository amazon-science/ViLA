...
@registry.register_model("blip2_t5_instruct_vqa_dist_ol")
class Blip2T5InstructVQADistOnline(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def forward(self, samples, temp):
        ...
        for j in range(image_embeds.size(1)):
            ...
            frame_atts_t5_t = torch.ones(frame_inputs_t5.size()[:-1], dtype=torch.long).to(image.device)
            atts_t5_t.append(frame_atts_t5_t)

        inputs_t5_t = torch.cat(inputs_t5_t, dim=1)
        atts_t5_t = torch.cat(atts_t5_t, dim=1)
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            inputs_embeds_t5 = self.t5_model.encoder.embed_tokens(input_tokens.input_ids)
            inputs_embeds = torch.cat([inputs_t5, inputs_embeds_t5], dim=1)
            ...
            # teacher
            inputs_embeds_t = torch.cat([inputs_t5_t, inputs_embeds_t5], dim=1)
            encoder_atts_t = torch.cat([atts_t5_t, input_tokens.attention_mask], dim=1)
            ...
            outputs_t = self.t5_model(
                inputs_embeds=inputs_embeds_t,
                attention_mask=encoder_atts_t,
                decoder_attention_mask=output_tokens.attention_mask,
                return_dict=True,
                labels=targets,
            )
            loss_t = outputs_t.loss
            ...
            loss = loss + loss_t + 0.001 * loss_dist
