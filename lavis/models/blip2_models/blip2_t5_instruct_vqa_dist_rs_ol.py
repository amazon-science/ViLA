...
# random sample
@registry.register_model("blip2_t5_instruct_vqa_dist_rs")
class Blip2T5InstructVQADistRs(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def forward(self, samples):
        ...
        s_image_embeds = []
        for i in range(self.model_frame):
            idx = random.randint(0, t//self.model_frame-1)
            s_image_embeds.append(image_embeds[:,idx+i*8:idx+i*8+1,:,:])

        s_image_embeds = torch.cat(s_image_embeds, dim=1)
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            # teacher
            inputs_embeds_t = torch.cat([inputs_t5_t, inputs_embeds], dim=1)
            ...
            loss = loss + loss_t + 0.5 * loss_dist
