...
@registry.register_model("blip2_fmr_tt") # frame-level moment retrieval
class Blip2FMRTT(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def forward(self, samples):
        ...
        for i in range(len(text_input)):
            text_input[i] = 'Question: What is in this photo? Answer: ' + ', '.join(
                text[self.frame_num * i:self.frame_num * (i + 1)]) + '. ' + text_input[i]
            #print(text_input[i])
        # tt
    ...
    def predict_answers(
        ...
        **kwargs
        ...
        text_input = ["Question: What is in this photo? Answer:" for _ in range(len(samples["qa_input"])*t)]

        input_tokens = self.t5_tokenizer(
            text_input, padding="longest", return_tensors="pt"
        ).to(image.device)

        #print("atts_t5, input_tokens.attention_mask", atts_t5.shape, input_tokens.attention_mask.shape)
    ...
    def lemmatizer(self):
        if self._lemmatizer is None:
            try:
                ...
                logging.error(
                    """
                    install spacy and en_core_web_sm model to apply lemmatization.
