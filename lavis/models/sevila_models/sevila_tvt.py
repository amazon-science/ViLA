...
@registry.register_model("sevila_tvt")
class SeViLATVT(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        self.vt_proj = nn.Linear(2048, 1408)
    ...
    def forward(self, samples,
        ...
        for i in range(len(samples['qa_input'])):
            samples['qa_input'][i] = 'Question: a photo of? Answer: ' + text[i] + '. ' + samples['qa_input'][i]
            print(samples['qa_input'][i])
        # tt
        ...
        # Finetune answerer with localizer
            ...
            with torch.cuda.amp.autocast(dtype=torch.float32):
                ...
                inputs_embeds_qa_p = self.vt_proj(inputs_embeds_qa)
                inputs_embeds_qa_p = torch.repeat_interleave(inputs_embeds_qa_p, 4, dim=0)

                #inputs_embeds_qa_p.expand(select_frames.shape[0], -1, -1)
                select_frames = torch.cat([select_frames, inputs_embeds_qa_p], dim=1)
                image_atts = torch.ones(select_frames.size()[:-1], dtype=torch.long).to(image.device)  # bt n c
                query_output_qa = self.Qformer.bert(
        ...
        **kwargs
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            if 'qa_vid' in self.task:
                ...
                text_input_qa = ["a photo of" for _ in range(len(samples["qa_input"]))]
