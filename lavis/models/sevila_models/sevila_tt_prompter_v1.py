...
#prompter
@registry.register_model("sevila_tt_pv1")
class SeViLATTPV1(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def forward(self, samples,
        ...
        #print('samples[qa_input] length:', len(samples['qa_input']))
        for i in range(len(samples['qa_input'])):
            samples['qa_input'][i] = 'Question: What is in this photo? Answer: ' + ', '.join(
                text[t * i:t * (i + 1)]) + '. ' + samples['qa_input'][i]
        # tt
        ...
        # Finetune answerer with localizer
            ...
            with torch.cuda.amp.autocast(dtype=torch.float32):
                ...
                #print('query_tokens_qa type', query_tokens_qa.dtype)
                #print('select_frames type', select_frames.dtype)
                query_output_qa = self.Qformer.bert(
    ...
    def generate(self,
        ...
        text = self.predict_answers(samples)
        # for i in range(len(samples['qa_input'])):
        #     samples['qa_input'][i] = 'Question: What is in this photo? Answer: ' + text[i] + '. ' + samples['qa_input'][i]
        for i in range(len(samples['qa_input'])):
            samples['qa_input'][i] = 'Question: What is in this photo? Answer: ' + ', '.join(
                text[self.frame_num * i:self.frame_num * (i + 1)]) + '. ' + samples['qa_input'][i]
        # tt
        ...
        **kwargs
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            outputs_loc = self.t5_model.generate(
                ...
                max_new_tokens=50, min_length=15, repetition_penalty=1.0,
            ...
            if 'qa_vid' in self.task:
                ...
                select_frames = torch.stack(select_frames, dim=0)  # b 4, n , -1
                #print('select_frames before,', select_frames.shape)
                select_frames = select_frames.reshape(-1, select_frames.shape[-2], select_frames.shape[-1])
                #print('select_frames,', select_frames.shape)
                image_atts = torch.ones(select_frames.size()[:-1], dtype=torch.long).to(image.device)  # bt n c
                query_tokens_qa = self.query_tokens.expand(select_frames.shape[0], -1, -1)
                query_output_qa = self.Qformer.bert(
                    query_embeds=query_tokens_qa, encoder_hidden_states=select_frames,
                ...
                #print('len(samples["qa_input"])', len(samples["qa_input"]))
                text_input_qa = ["Question: What is in this photo? Answer:" for _ in range(len(samples["qa_input"])*4)]
                #print('text_input_qa', len(text_input_qa))
                ...
                #print("[inputs_t5_qa, inputs_embeds_qa]", inputs_t5_qa.shape, inputs_embeds_qa.shape)
                inputs_embeds_qa = torch.cat([inputs_t5_qa, inputs_embeds_qa], dim=1)
                encoder_atts_qa = torch.cat([atts_t5_qa, input_tokens_qa.attention_mask], dim=1)
    ...
    #
    #     text_input = ["please give a detailed description of this photo" for _ in range(len(samples["qa_input"]) * self.frame_num)]
    #
